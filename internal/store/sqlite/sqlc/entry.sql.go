...
const existsActiveBankAccountByID = `-- name: ExistsActiveBankAccountByID :one
SELECT EXISTS(
    SELECT 1
    FROM bank_accounts
...
func (q *Queries) ExistsActiveBankAccountByID(ctx context.Context, id int64) (int64, error) {
	row := q.db.QueryRowContext(ctx, existsActiveBankAccountByID, id)
	var column_1 int64
...
WHERE deleted_at_utc IS NULL
  ...
  AND (?3 IS NULL OR bank_account_id = ?3)
  AND (?4 IS NULL OR transaction_date_utc >= ?4)
  AND (?5 IS NULL OR transaction_date_utc <= ?5)
  AND (?6 IS NULL OR (note IS NOT NULL AND instr(lower(note), lower(?6)) > 0))
...
type ListActiveEntriesParams struct {
	...
	BankAccountID interface{} `json:"bank_account_id"`
...
WHERE tl.deleted_at_utc IS NULL
  ...
  AND (?3 IS NULL OR t.bank_account_id = ?3)
  AND (?4 IS NULL OR t.transaction_date_utc >= ?4)
  AND (?5 IS NULL OR t.transaction_date_utc <= ?5)
  AND (?6 IS NULL OR (t.note IS NOT NULL AND instr(lower(t.note), lower(?6)) > 0))
...
type UpdateEntryByIDParams struct {
	...
	CategoryID            sql.NullInt64  `json:"category_id"`
	ClearBankAccount      interface{}    `json:"clear_bank_account"`
	SetBankAccountID      interface{}    `json:"set_bank_account_id"`
	BankAccountID         sql.NullInt64  `json:"bank_account_id"`
...
func (q *Queries) UpdateEntryByID(ctx context.Context, arg UpdateEntryByIDParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, updateEntryByID,
		...
		arg.SetCategoryID,
		arg.CategoryID,
		arg.ClearBankAccount,
		arg.SetBankAccountID,
		arg.BankAccountID,
		arg.ClearNote,
		arg.SetNote,
		arg.Note,
